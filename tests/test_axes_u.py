...
def test_compute_vertical_axes_primary_and_opening_axes_present():
    ...
    v = compute_vertical_axes(L=structure.footprint.length, W=structure.footprint.width, b_max=1.5, openings=list(openings_final))
...
def test_compute_vertical_axes_secondary_limits_spans_outside_openings():
    ...
    b_max = 1.5
    v = compute_vertical_axes(L=structure.footprint.length, W=structure.footprint.width, b_max=b_max, openings=list(openings_final))
    all_axes = v["S"]["all"]
